...
int main()
{
    ...
    if((char_num >= 97) && (char_num <= 122))               //first case for lower case of letter
    {
        for(int i = char_num; i <= 123; i++)
        {
            ...
            printf("%c ",i);
            counter++;
            //loop continues for 26 times to print all letters, when count reaches 26 it breaks the loop
            if(counter == 26)
            {
                break;
            }
